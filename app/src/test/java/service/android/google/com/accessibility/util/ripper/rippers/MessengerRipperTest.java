...
public class MessengerRipperTest {

    private MessengerRipper messengerRipper;
    ...
    public void setUp() throws Exception {
        ...
        messengerRipper = new MessengerRipper();
    }
    ...
    public void test_getWindowInfoEventFromAccessibilityNodeInfo_shouldReturnNullWhenSizeOfContactNameIsZero() throws Exception {
        when(nodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.CONTACT_NAME_RES_ID))).thenReturn(Collections.EMPTY_LIST);
        assertNull(messengerRipper.getWindowInfoEventFromAccessibilityNodeInfo(nodeInfo));
    }

    @Test
    public void test_getWindowInfoEventFromAccessibilityNodeInfo_shouldHaveCreatedContactPersonObject() throws Exception {
        when(nodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.CONTACT_NAME_RES_ID))).thenReturn(asList(contactNameNodeInfo));
        when(contactNameNodeInfo.getContentDescription()).thenReturn(contactPersonFullName);
        messengerRipper.getWindowInfoEventFromAccessibilityNodeInfo(nodeInfo);
    ...
    public void test_scrapeChatMessages_shouldReturnChatEventWithCorrectPackageName() throws Exception {
        when(nodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.CONTACT_NAME_RES_ID))).thenReturn(asList(contactNameNodeInfo));
        when(contactNameNodeInfo.getContentDescription()).thenReturn(contactPersonFullName);
        when(nodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_CONTAINER_RES_ID))).thenReturn(asList(messageContainerNodeInfo));
        when(messageContainerNodeInfo.getParent()).thenReturn(parentNodeInfo);

        final ChatEvent windowInfoEventFromAccessibilityNodeInfo = messengerRipper.getWindowInfoEventFromAccessibilityNodeInfo(nodeInfo);
    ...
    public void test_getTextFromMessageContainer_shouldReturnChatEventWithText_VIDEO_type() throws Exception {
        ...
        when(nodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_CONTAINER_RES_ID))).thenReturn(asList(messageContainerNodeInfo));
        when(messageContainerNodeInfo.getParent()).thenReturn(parentNodeInfo);
        when(parentNodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_VIDEO_RES_ID))).thenReturn(asList(videoContainerNodeInfo));
    ...
    public void test_getTextFromMessageContainer_shouldReturnChatEventWithText_ThirdParty_type() throws Exception {
        ...
        when(nodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_CONTAINER_RES_ID))).thenReturn(asList(messageContainerNodeInfo));
        when(messageContainerNodeInfo.getParent()).thenReturn(parentNodeInfo);
        when(parentNodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_THIRD_PARTY_NAME_RES_ID))).thenReturn(asList(thirdPartyAppNameContainerNodeInfo));
    ...
    public void test_getTextFromMessageContainer_shouldReturnChatEventWithText_LINK_type() throws Exception {
        ...
        when(parentNodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_LINK_TITLE_TEXT))).thenReturn(asList(messageLinkTitleNodeInfo));
        when(parentNodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_LINK_DESCRIPTION_TEXT))).thenReturn(asList(messageLinkDescriptionNodeInfo));
        when(parentNodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_LINK_SOURCE))).thenReturn(asList(messageLinkSourceNodeInfo));
    ...
    public void test_getTextFromMessageContainer_shouldReturnChatEventWithText_TEXT_type() throws Exception {
        ...
        when(nodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_CONTAINER_RES_ID))).thenReturn(asList(messageContainerNodeInfo));
        when(messageContainerNodeInfo.getParent()).thenReturn(parentNodeInfo);
        when(parentNodeInfo.findAccessibilityNodeInfosByViewId(messengerRipper.getFQResID(MessengerRipper.MESSAGE_TEXT_RES_ID))).thenReturn(asList(textContainerNodeInfo));
